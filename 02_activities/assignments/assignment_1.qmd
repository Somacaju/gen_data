...
plink2 --bfile ./02_activities/data/gwa.qc.A1 \
   ...
   --freq \
   --out ./02_activities/data/gwa_qc_freq_A1
```

```{r}
freq <- fread("./02_activities/data/gwa_qc_freq_A1.afreq")
freq 
```
...
# MAF
```{r}
freq[, MAF := pmin(ALT_FREQS, 1 - ALT_FREQS)]
freq
```
...
plink2 --bfile ./02_activities/data/gwa.qc.A1 \
   --hardy \
   --out ./02_activities/data/gwa_qc_hwe_A1
```
...
```{r, message=FALSE, warning=FALSE}         
geno_subset_A1.1 <- fread("./02_activities/data/geno.additive_A1.raw")
```

```{r, message=FALSE, warning=FALSE}         
colnames(geno_subset_A1.1)
```

```{r, message=FALSE, warning=FALSE}         
geno_subset_A1.1 <- na.omit(geno_subset_A1.1)
geno_subset_A1.1$PHENOTYPE <- geno_subset_A1.1$PHENOTYPE - 1
```

```{r, message=FALSE, warning=FALSE}         
linearmodel_add <-lm(PHENOTYPE ~ rs1861_C, data = geno_subset_A1.1)
summary(linearmodel_add)
```
...
# Beta coefficient:  0.96698. For each additional copy of the C allele at rs1861,  the phenotype increases in approximately 0.97 units in the phenotype
...
```{r, message=FALSE, warning=FALSE}         
ggplot(geno_subset_A1.1, aes(x = rs1861_C, y = PHENOTYPE)) +
  geom_point() +
  geom_smooth(method= "lm", se = FALSE) +
  labs(x= "Genotype (rs1861)", y = "Phenotype")
```

(iv) Convert the genotype coding for rs1861 to recessive coding.

```{r, message=FALSE, warning=FALSE}         
geno_subset_A1.1$rs1861_rec <- ifelse(geno_subset_A1.1$rs1861_C == 2, 1, 0)
table(geno_subset_A1.1$rs1861_rec)
```
...
```{r, message=FALSE, warning=FALSE}         
linearmodel_rec <-lm(PHENOTYPE ~ rs1861_rec, data = geno_subset_A1.1)
summary(linearmodel_rec)
```

# Answer: The p-value is <2e-16

(vi) Plot the scatterplot of phenotype versus the recessive-coded genotype of rs1861. Add the regression line to the plot.

```{r, message=FALSE, warning=FALSE}         
ggplot(geno_subset_A1.1, aes(x = rs1861_rec, y = PHENOTYPE)) +
  geom_point() +
  geom_smooth(method= "lm", se = FALSE) +
  labs(x= "Genotype (rs1861)", y = "Phenotype")
```
...
```{r, message=FALSE, warning=FALSE} 
AIC(linearmodel_add, linearmodel_rec)
```

# AIC:
df      AIC
linearmodel_add  3 10988.06
linearmodel_rec  3 11000.02
> 

# Answer: According to the AIC, the additive model fits better as it has a lower AIC value (10988.06), compared to the recessive model, indicating a better balance between model fit and complexity.
